...
def gauk_testu_statistika(testai):
    ...
    for testo_variantas, zmoniu_rez in testai.items():
        ...
        islaikymai = [round(sum(zmogaus_rez) / TASKU_MAKSIMUMAS * 100) >= ISLAIKYMO_PROCENTAS for zmogaus_rez in zmoniu_rez]
        islaikiusiu_zmoniu_sk = islaikymai.count(True)
...
'''Atidaromas duomenu failas, perskaitomas ir issaugomas i dictionary tipo duomenu struktura '''
testai = {}
